...
<img src="assets/images/bar.jpg" width="880" alt="EulerAgent Banner"/>
...
> 📌 **Official Channel** — This GitHub repository is the **only** official source of EulerAgent.
> We have no affiliation with any third-party website using the EulerAgent name.
...
**EulerAgent** is a minimal, self-evolving autonomous agent framework. Its core is just **~3K lines of code**. Through **9 atomic tools + a ~100-line Agent Loop**, it grants any LLM system-level control over a local computer — covering browser, terminal, filesystem, keyboard/mouse input, screen vision, and mobile devices (ADB).

> Design philosophy — **don't preload skills, evolve them.**

Every time EulerAgent solves a new task, it automatically crystallizes the execution path into a reusable **Skill**. The longer you use it, the more skills accumulate — forming a personal skill tree grown entirely from 3K lines of seed code.

> 🤖 **Self-Bootstrap Proof** — Everything in this repository, from installing Git and running `git init` to every commit message, was completed autonomously by EulerAgent. The author never opened a terminal once.
...
> ⚠️ **Python version**: use **Python 3.11 or 3.12**. **Do not** use Python 3.14 — it is incompatible with `pywebview` and a few other EA dependencies.
>
...
This installs EulerAgent with an isolated Python environment and Git, then downloads a ready-to-run package.
...
> 💡 EulerAgent is meant to grow its environment **through the Agent itself**, not by pre-installing every possible package.
...
3. If it still looks broken, ask EA itself to fix it:
...
EulerAgent also supports IM frontends such as Telegram, WeChat, QQ, Feishu / Lark, WeCom, and DingTalk.
...
> For detailed setup, ask EulerAgent itself.
...
EulerAgent accomplishes complex tasks through **Layered Memory × Minimal Toolset × Autonomous Execution Loop**, continuously accumulating experience during execution.
...
> *EulerAgent provides only **9 atomic tools**, forming the foundational capabilities for interacting with the outside world.*
...
Via `code_run`, EulerAgent can dynamically install Python packages, write new scripts, call external APIs, or control hardware at runtime — crystallizing temporary abilities into permanent tools.

<div align="center">
  <img src="assets/images/workflow.jpg" alt="EulerAgent Workflow" width="420"/>
  <br/><em>EulerAgent Workflow Diagram</em>
</div>
...
This is what fundamentally distinguishes EulerAgent from every other agent framework.
...
| Feature | **EulerAgent** | OpenClaw | Claude Code |
| :--- | :---: | :---: | :---: |
...
We evaluate EulerAgent across **five dimensions**:

| # | Dimension | Question | Benchmarks |
| :---: | :--- | :--- | :--- |
| 1 | **Task Completion & Token Efficiency** | Can EA complete hard tasks more cheaply than leading agents? | SOP-Bench, Lifelong AgentBench, RealFin-Benchmark |
...
<table>
  <tr>
    <td align="center" width="50%">
      ...
      <sub><b>Tool-use efficiency radar.</b> EA dominates token, request, and tool-call axes while preserving quality across four task dimensions.</sub>
    </td>
    <td align="center" width="50%">
      <img src="assets/images/result_convergence.png" width="100%" alt="Cross-task self-evolution convergence"/><br/>
      <sub><b>Cross-task self-evolution.</b> Second- and third-run EA executions converge to a stable low-cost regime across eight web tasks, while OpenClaw shows no such convergence.</sub>
    </td>
  </tr>
</table>
...
- **2026-05-15** — 🖥️ **Desktop GUI released**. One-line installs ship a ready-to-run desktop app (`frontends/EulerAgent.exe`). Developers launch via `python launch.pyw`.
...
- **2026-03-08** — [Released "Dintal Claw" — a EulerAgent-powered government-affairs bot](https://mp.weixin.qq.com/s/eiEhwo-j6S-WpLxgBnNxBg).
- **2026-03-01** — [Featured by Jiqizhixin (机器之心)](https://mp.weixin.qq.com/s/uVWpTTF5I1yzAENV_qm7yg).
- **2026-01-16** — EulerAgent **V1.0** public release.
...
You're also welcome to join the **EulerAgent Community Group** for discussion, feedback, and co-building 👏
...
> *Disclaimer: This project does not build or operate any commercial website. Apart from DintalClaw, no institution, organization, or individual is currently officially authorized to conduct commercial activities under the EulerAgent name.*
...
**EulerAgent** 是一个极简、可自我进化的自主 Agent 框架。核心仅 **~3K 行代码**，通过 **9 个原子工具 + ~100 行 Agent Loop**，赋予任意 LLM 对本地计算机的系统级控制能力，覆盖浏览器、终端、文件系统、键鼠输入、屏幕视觉及移动设备（ADB）。

> 设计哲学 —— **不预设技能，靠进化获得能力。**

每解决一个新任务，EulerAgent 就将执行路径自动固化为 Skill，供后续直接调用。使用时间越长，沉淀的技能越多，形成一棵完全属于你、从 3K 行种子代码生长出来的专属技能树。

> 🤖 **自举实证** — 本仓库的一切，从安装 Git、`git init` 到每一条 commit message，均由 EulerAgent 自主完成。作者全程未打开过一次终端。
...
> 💡 EulerAgent 更推荐由 **Agent 在使用中自举环境**，而不是预先手动装完整依赖。

📖 完整引导流程见 [`docs/GETTING_STARTED.md`](docs/GETTING_STARTED.md)
📖 新手图文版：[飞书文档](https://my.feishu.cn/wiki/CGrDw0T76iNFuskmwxdcWrpinPb)
📘 完整入门教程（Datawhale 出品）：[Hello EulerAgent](https://datawhalechina.github.io/hello-generic-agent/) · [GitHub](https://github.com/datawhalechina/hello-generic-agent)
...
3. 仍然显示异常时，可以让 EA 自己修一遍，参考 Prompt：
...
EulerAgent 支持 Telegram、微信、QQ、飞书 / Lark、企业微信、钉钉等 IM 前端。
...
EulerAgent 通过 **分层记忆 × 最小工具集 × 自主执行循环** 完成复杂任务，并在执行过程中持续积累经验。
...
> *EulerAgent 仅提供 **9 个原子工具**，构成与外部世界交互的基础能力。*
...
通过 `code_run`，EulerAgent 可在运行时动态安装 Python 包、编写新脚本、调用外部 API 或控制硬件，将临时能力固化为永久工具。

<div align="center">
  <img src="assets/images/workflow.jpg" alt="EulerAgent 工作流程" width="420"/>
  <br/><em>EulerAgent 工作流程图</em>
</div>
...
这是 EulerAgent 区别于其他 Agent 框架的根本所在。
...
| 特性 | **EulerAgent** | OpenClaw | Claude Code |
| :--- | :---: | :---: | :---: |
...
我们从 **五大维度** 评测 EulerAgent：

| # | 维度 | 核心问题 | 使用的基准 |
| :---: | :--- | :--- | :--- |
| 1 | **任务完成度与 Token 效率** | EA 能否以更低成本完成高难度任务？ | SOP-Bench、Lifelong AgentBench、RealFin-Benchmark |
...
<table>
  <tr>
    <td align="center" width="50%">
      ...
      <sub><b>工具使用效率雷达图。</b>EA 在 Token、请求数和工具调用轴上全面领先，同时在四个任务维度上保持质量。</sub>
    </td>
    <td align="center" width="50%">
      <img src="assets/images/result_convergence.png" width="100%" alt="跨任务自我进化收敛曲线"/><br/>
      <sub><b>跨任务自我进化。</b>EA 的第二轮和第三轮执行在 8 个 Web 任务上收敛至稳定的低成本区间。</sub>
    </td>
  </tr>
</table>
...
- **2026-05-15** — 🖥️ **桌面 GUI 发布**。一键安装会自带可直接运行的桌面端（`frontends/EulerAgent.exe`），开发者也可用 `python launch.pyw` 启动。
...
- **2026-03-08** — [发布以 EulerAgent 为核心的"政务龙虾" Dintal Claw](https://mp.weixin.qq.com/s/eiEhwo-j6S-WpLxgBnNxBg)。
- **2026-03-01** — [被机器之心报道](https://mp.weixin.qq.com/s/uVWpTTF5I1yzAENV_qm7yg)。
- **2026-01-16** — EulerAgent **V1.0** 公开版本发布。
...
也欢迎加入 **EulerAgent 体验交流群**，一起交流、反馈、共建 👏
...
> *声明：本项目未构建任何商业站点；除 DintalClaw 外，目前未官方授权任何机构、组织或个人以 EulerAgent 名义从事商业活动。*
