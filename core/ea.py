...
def code_run(code, code_type="python", timeout=60, cwd=None, code_cwd=None, stop_signal=None, maxlen=10000):
    ...
    if code_type in ["python", "py"]:
        ...
        cr_header = os.path.join(script_dir, '../assets/scripts', 'code_run_header.py')
...
class EulerAgentHandler(BaseHandler):
    '''Euler Agent 工具库，包含多种工具的实现。工具函数自动加上了 do_ 前缀。实际工具名没有前缀。'''
...
''' + get_global_memory()
        ...
        path = os.path.join(script_dir, '../memory/memory_management_sop.md')
...
def get_global_memory():
    ...
    try:
        ...
        with open(os.path.join(script_dir, f'../assets/template/insight_fixed_structure{suffix}.txt'), 'r', encoding='utf-8') as f: structure = f.read()
