...
class EulerAgentAcpBridge:
    ...
    def handle_session_prompt(self, req_id: Any, params: Dict[str, Any]) -> None:
        ...
        def run_prompt() -> None:
            ...
            try:
                ...
                eprint("[EulerAgent ACP] prompt thread failed:", traceback.format_exc())
            finally:
                with session.prompt_lock:
    ...
    def handle_message(self, msg: Dict[str, Any]) -> None:
        ...
        try:
            ...
            eprint("[EulerAgent ACP] request handler failed:", traceback.format_exc())
            if req_id is not None:
                self.write_message(
    ...
    def serve(self) -> None:
        eprint("[EulerAgent ACP] bridge started")
        ...
        for raw_line in stdin:
            ...
            if self._shutdown:
                break
        eprint("[EulerAgent ACP] bridge stopped")


def main() -> int:
    parser = argparse.ArgumentParser(description="EulerAgent ACP bridge over stdio")
    parser.add_argument("--llm-no", type=int, default=0, help="LLM index for EulerAgent")
    args = parser.parse_args()
    bridge = EulerAgentAcpBridge(llm_no=args.llm_no)
    bridge.serve()
    return 0
