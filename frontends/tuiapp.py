"""Textual terminal UI for EulerAgent.
...
MVP design notes:
- One TUI manages multiple EulerAgent instances.
- EulerAgent.put_task() returns a per-task display_queue; the TUI records a task_id for every submit.
- Agent.run() and display_queue.get() run in daemon threads; UI updates are posted via App.call_from_thread().
- Multiple sessions may run concurrently, but EulerAgent still shares project temp/memory/tool globals.
"""
...
def fold_turns(text: str) -> list[dict[str, str]]:
    """Split EulerAgent turn output into text/fold segments.
...
class EulerAgentTUI(App[None]):
    """Textual app that manages multiple EulerAgent sessions."""
    ...
    def on_mount(self) -> None:
        ...
        self._system("Welcome to EulerAgent TUI. Type /help for commands.")
    ...
    def _cmd_help(self, args: list[str]) -> None:
        self._system(
            ...
            "/quit - exit TUI\n\n"
            "Unknown slash commands (for example /session.x=... or /resume) are sent to EulerAgent."
        )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Textual TUI for EulerAgent")
    return parser
