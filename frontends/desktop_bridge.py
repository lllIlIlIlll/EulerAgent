#!/usr/bin/env python3
"""
EulerAgent Web2 Bridge.

Clear split:
1) AgentManager: owns EulerAgent instances, sessions and histories.
...
class AgentManager:
    ...
    def make_agent(self, sess: Session):
        ...
        try:
            ...
            GA = getattr(agentmain, "EulerAgent")
            agent = GA()
            agent.inc_out = True
            agent.verbose = True
    ...
    def run_agent_turn(self, sess: Session, prompt: str, images: Optional[list] = None):
        try:
            ...
            if hasattr(agent, "put_task"):
                ...
                full = "EulerAgent object has no put_task/run method"
            if not full:
                full = "(completed)"
            if sess.cancel_requested:
                with self.lock:
...
if __name__ == "__main__":
    ...
    print(f"EulerAgent Web2 bridge: http://{host}:{port}  ws://{host}:{port}/ws", file=sys.stderr)
