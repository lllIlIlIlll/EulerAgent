"""EulerAgent TUI v2 — Textual app with refined visual style.
...
def _ensure_tui_deps() -> None:
    ...
    Keeps `ea-cli` working on a fresh Python (Windows / macOS / Linux) where
...
def _grab_clipboard_file() -> Optional[tuple[str, bool]]:
    ...
    if isinstance(data, Image.Image):
        try:
            out_dir = os.path.join(tempfile.gettempdir(), "euleragent_tui_clipboard")
...
def render_status_chip(busy: bool, elapsed: int = 0) -> Text:
    """`✦ EulerAgent` identity chip. Brightens green when any session is busy.
    ...
    chip.append("EulerAgent", style=f"bold {C_GREEN}" if busy else f"bold {C_FG}")
    return chip
...
def render_topbar(session_name: str, status: str, model: str, tasks_running: int,
    ...
    # EulerAgentTUI._update_terminal_title.
...
class EulerAgentTUI(App[None]):
    ...
    def __init__(self, agent_factory: Optional[AgentFactory] = None) -> None:
        ...
        try:
            ...
            pass
        # Patch EulerAgent for /review in case chatapp_common didn't wire it.
        try:
            from core.agentmain import EulerAgent as _GA
    ...
    def on_mount(self) -> None:
        ...
        self._system("Welcome to EulerAgent TUI. 按 / 唤起命令面板，Ctrl+N 新建会话。")
    ...
    def _install_ask_user_hook(self, sess: AgentSession) -> None:
        ...
        where `parent` is the EulerAgent — so the dict lives on the agent.
        """
        agent = sess.agent
        try:
    ...
    def _cmd_rename(self, args, raw):
        ...
        # writes to (see agentmain.EulerAgent.__init__); exclude its basename
    ...
    def _update_terminal_title(self) -> None:
        ...
        if busy:
            ...
            title = f"{glyph} {name} · EulerAgent"
        else:
            title = f"{name} · EulerAgent"
...
def build_arg_parser() -> argparse.ArgumentParser:
    return argparse.ArgumentParser(description="EulerAgent TUI v2 (refined visual style)")
