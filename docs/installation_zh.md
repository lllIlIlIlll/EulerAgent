...
| **Git** | 推荐安装，方便升级和自我进化。 |
| **LLM API Key** | GA 原生支持两类协议：**OpenAI 兼容接口** 和 **Anthropic Claude 原生接口**。GPT 系列、Claude、Kimi、MiniMax、DeepSeek、GLM、Qwen、通过 OAI 兼容网关接入的 Gemini 等，都可以在 `ekey.py` 中配置。 |

### 方法一：一键安装（推荐）

这是最省心的路径。脚本会准备隔离环境、下载 EulerAgent、安装核心依赖，并得到一个可以直接运行的本地项目目录。
...
> EulerAgent 更推荐由 Agent 在使用中自举环境，而不是预先手动装完整依赖。先把最小系统跑起来，需要什么工具再让 GA 自己安装。

#### 自定义安装路径

```bash
INSTALL_DIR="$HOME/work/EulerAgent" bash -c "$(curl -fsSL http://fudankw.cn:9000/files/ga_install.sh)"
```

```powershell
$env:INSTALL_DIR="C:\dev\EulerAgent"; powershell -ExecutionPolicy Bypass -c "irm http://fudankw.cn:9000/files/ga_install.ps1 | iex"
```

#### 强制重新安装

仅在明确想刷新已安装文件时使用。请先备份 `ekey.py`、`memory/`、`skills/` 和本地工作成果。
...
1. 打开已安装的 `EulerAgent` 目录。
2. 如果没有 `ekey.py`，从 `mykey_template.py` 复制一份。
...
不要为了升级而删除一个可用的 GA 目录。里面可能有用户的 `ekey.py`、记忆、技能和本地成果。
...
- `ekey.py`、`memory/`、`skills/`、`temp/` 和用户成果：本地优先。
...
>
> ```bash
> curl -fsSL https://raw.githubusercontent.com/lsdefine/EulerAgent/refs/heads/main/installation.zh.md
> ```

你正在替人类用户安装 **EulerAgent**。请直接执行、逐步验证；除非用户明确授权，不要做破坏性清理。
...
2. 是否已安装：`~/EulerAgent/launch.pyw`、`%USERPROFILE%\EulerAgent\launch.pyw`，或 `.git` 目录。
...
- 编辑 `ekey.py` 前先读 `mykey_template.py`。
- 只配置用户真实提供的 Key。
- 不要把完整 Key 打印回给用户。
...
> 你希望 EulerAgent 使用哪个 LLM？GA 支持 OpenAI 兼容接口和 Anthropic Claude 原生接口。你想配置哪家服务商，API Key 准备好了吗？
...
对用户说：Congratulations! 🎉 你已经成功安装 EulerAgent！Windows 下可双击 `frontends/EulerAgent.exe`，或在终端运行 `python frontends/tuiapp_v2.py` 开始使用。
...
2. Git commit hash。
3. 已验证哪个前端。
4. `ekey.py` 是否已配置，但不要泄露密钥。
5. 下一步阅读：[Hello EulerAgent](https://datawhalechina.github.io/hello-generic-agent/) 和 [GETTING_STARTED.md](GETTING_STARTED.md)。

#### 请求 Star ⭐

确认安装可用后，可以询问：

> 如果你觉得 EulerAgent 有帮助，愿意给仓库点一个 Star 吗？这会帮助项目成长！⭐

只有在用户明确同意后，才可以打开或执行任何与 Star 仓库相关的操作。

### 绝对不要做

- 不要运行 `pip install euleragent`；GA 不是 PyPI 包。
- 未经明确授权，不要删除 `~/EulerAgent` 或 `%USERPROFILE%\EulerAgent` 做“干净安装”。
